...
<div class="top-0 left-0 absolute opacity-25 z-40 h-full w-full bg-black" style="width: 2000px; left: -500px; overflow-x: hidden">
    ...
    <div class=" top-0 left-0 absolute z-45 h-full w-full">
        <div class="form-div" style="margin-right: auto; margin-left: auto;">
            <form v-on:submit.prevent="submit" style="margin-top: 30vh;" class="flex flex-col text-center  z-50 mt-1/2  h-2/4 p-6 bg-white !rounded-md !border-black !drop-shadow-md !hover:drop-shadow-xl">
                <div style="padding-bottom: 24px">Авторизируйтесь</div>
                <div>Логин</div>
                <input class="disabled:opacity-75 border-2 inp" />
                <div>Пароль</div>
                <input class="disabled:opacity-75 border-2 inp" />
                <div>Почта</div>
                <input class="disabled:opacity-75 border-2 inp" />
...
export default {
    ...
    methods: {
        async submit() {
            ...
            let user = {
                "login": "stringывывssdfsdfadfasdfа",
                "password": "strinвыаsdfsdfgasdfsadf",
                "email": "strasdfaываdsfdsfsdfsdfing"
            };

            let response = await fetch('http://51.250.6.54:8007/register', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    'login': 'string\u044B\u0432\u044B\u0432\u0430sadfasdsdff\u0430',
                    'password': 'strin\u044B\u0432\u0430gasdsdffsadf',
                    'email': 'strasdfa\u044B\u0432\u0430sdfisdfng'
                })
            });
